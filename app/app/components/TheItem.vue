...
const timeDiffHours = computed(() => {
  ...
  const diff = Math.abs(
    new Date(props.prev.created_at).getTime() -
    ...
    return h("span", [
      ...
      ...mergedParts.map((part, i) => {
        if (i) {
          return h(
            "span",
            { class: `${color} font-bold px0.8 mx--0.8 rounded` },
            part
          );
        }
        return part;
  ...
  >
    ...
    >
      <img
        ...
        bg-gray:5
        :class="
          item.isOrg === false && !logoOverrides[props.item.repo]
            ? 'rounded-full'
            : 'rounded'
        "
      />
      ...
      >
        <img
          ...
          alt="Sub logo"
        />
        <div v-else :class="subImage" ma h-4.5 w-4.5 />
      </div>
    </a>
    ...
    <div flex="~ row gap-2 auto items-center" lt-sm="flex-col items-start">
      <div flex-auto flex="~ col">
        <div flex="~ gap-2 items-center">
          ...
          >
            <div text-sm op50>
              {{ item.repo.split("/")[0] }}<span op75>/</span>
            </div>
      ...
      <div flex="~ col items-end" lt-sm="flex-row gap-2" text-end>
        <a
          font-mono
          :href="`https://github.com/${item.repo}/releases/tag/${item.package}@${item.version}`"
          target="_blank"
        >
          <HighlightedVersion />
        </a>
        <time text-sm op50 :datatime="new Date(item.created_at).toString()">{{
